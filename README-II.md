...
3. [🎴 Understanding the Coin Signal Cards (Visual Guide)](#3-understanding-the-coin-signal-cards-visual-guide)
4. [The "Team of Experts": Multi-Agent AI Architecture](#4-the-team-of-experts-multi-agent-ai-architecture)
5. [The 7-Step Journey of a Trade Signal](#5-the-7-step-journey-of-a-trade-signal)
6. [Multi-Timeframe Analysis (MTFA) Demystified](#6-multi-timeframe-analysis-mtfa-demystified)
7. [The 18 Technical Indicators Explained](#7-the-18-technical-indicators-explained)
8. [10 Macro Patterns & 9 Candlestick Patterns](#8-10-macro-patterns--9-candlestick-patterns)
9. [Smart Money Concepts (SMC / ICT) & Elliott Wave Theory](#9-smart-money-concepts-smc--ict--elliott-wave-theory)
10. [The 20 Deterministic Strategies & Consensus Math](#10-the-20-deterministic-strategies--consensus-math)
11. [The Unbreakable Digital Shield: The 8 Risk Rules](#11-the-unbreakable-digital-shield-the-8-risk-rules)
12. [Money & Risk Management Demystified](#12-money--risk-management-demystified)
13. [System Blueprint & Directory Map](#13-system-blueprint--directory-map)
14. [The Web Dashboard, Live Scanning & User Experience](#14-the-web-dashboard-live-scanning--user-experience)
15. [Summary Cheat Sheet & FAQ](#15-summary-cheat-sheet--faq)
...
* **The Telemetry Progress Monitor & Cockpit Screen (FastAPI & WebSockets)** streams live glowing dials, a real-time progress bar across all 50 coins, exact analysis timestamps, and interactive execution buttons to the team boss (you!).
...
| **Leverage (e.g., 5x, 10x, 20x)** | Multiplying your position size using borrowed funds. | Riding a bicycle (1x) vs. driving a turbocharged racecar (10x). Speed multiplies, but mistakes are magnified! |
...
## 3. 🎴 Understanding the Coin Signal Cards (Visual Guide)

When you look at the dashboard, each monitored cryptocurrency pair is displayed as an individual signal card. Here is what every single line and badge means:

```text
┌────────────────────────────────────────────────────────────────────────────────────────┐
│ 1. AVAXUSDT  [FUTURES]                                      [RISK GATE PASSED]         │
├────────────────────────────────────────────────────────────────────────────────────────┤
│ 2. 🕒 Analyzed: 2026-08-17 14:43:43                                            2m ago  │
├────────────────────────────────────────────────────────────────────────────────────────┤
│ 3. SIGNAL ACTION                                                           CONFIDENCE  │
│    SELL_SHORT                                                                     90%  │
├────────────────────────────────────────────────────────────────────────────────────────┤
│ 4. 🕒 ANALYZED TIMEFRAMES                                            100% Confluence   │
│    [ 🔴 15m BEAR ]                [ 🔴 1h BEAR ]                 [ 🔴 4h BEAR ]        │
├────────────────────────────────────────────────────────────────────────────────────────┤
│ 5. ENTRY                       TAKE PROFIT                                  STOP LOSS  │
│    $6.33                       $5.96                                            $6.51  │
├────────────────────────────────────────────────────────────────────────────────────────┤
│ 6. POSITION SIZING (USER BET)                                                          │
│    Margin Bet: $10                                                Pos Size: $100       │
│    Max Risk (SL): -$2.93                                          Target (TP): +$5.86  │
├────────────────────────────────────────────────────────────────────────────────────────┤
│ 7. Gross R:R: 2:1              Net R:R: 1.89:1                                         │
│    Net EV: +$1.34              Leverage: 10x                                           │
│    Sug. Leverage: [ 29x ✨ ]                                                           │
├────────────────────────────────────────────────────────────────────────────────────────┤
│ 8. • Tech: SELL_SHORT (66%)                                                            │
│    • News: BEARISH (-0.50)                                                             │
│    • Fund: 28/100                                                                      │
├────────────────────────────────────────────────────────────────────────────────────────┤
│ 9. [RISK GATE REJECTIONS (Only shown if Blocked)]:                                     │
│    • RULE 7 - HOLD Safety: HOLD signals are never executable                           │
├────────────────────────────────────────────────────────────────────────────────────────┤
│ 10. 🔍 Deep Dive                ⚡ Re-analyze                             🎯 Place Trade│
└────────────────────────────────────────────────────────────────────────────────────────┘
```

### Breakdown of the 10 Visual Sections:

#### 1. Header Bar: Coin, Market & Risk Gate Status
* **Coin Symbol & Market Badge** (`AVAXUSDT [FUTURES]` or `DOTUSDT [SPOT]`): Shows the asset and whether it's Spot or Perpetual Futures.
* **Risk Gate Status**:
  * `RISK GATE PASSED` (Green): The trade passed all 8 mathematical safety rules and is executable.
  * `RISK GATE BLOCKED` (Red): One or more safety rules failed (e.g. market is in neutral HOLD, or counter to trend), preventing you from risking money.

#### 2. Analysis Timestamp Bar
* **Exact Time (`Analyzed: 2026-08-17 14:43:43`)**: The exact timestamp when this coin was last scanned.
* **Relative Age (`2m ago`, `just now`)**: How fresh the analysis is. This automatically updates live in your browser every 15 seconds.

#### 3. Signal Action & Confidence Score
* **Action Banner**:
  * `BUY_LONG` (Green): Market setup suggests price is likely to go **UP**.
  * `SELL_SHORT` (Red): Market setup suggests price is likely to go **DOWN**.
  * `HOLD` (Gray/Slate): No high-probability edge detected; staying on the sidelines.
* **Confidence %** (`90%`, `27%`): How strongly the 20 trading strategies and AI specialist agents agree on the setup.

#### 4. Multi-Timeframe Confluence (MTFA)
* **Confluence Badge** (`100% Confluence`): How well the short, medium, and long-term charts agree.
* **Timeframe Pills**:
  * `15m BEAR`: The 15-minute trigger chart shows downward momentum.
  * `1h BEAR`: The 1-hour primary setup chart shows downward momentum.
  * `4h BEAR`: The 4-hour macro trend chart confirms the overarching downward tide.
  * When all 3 are red (or all 3 are green), the signal receives an automatic **+15% confidence boost**!

#### 5. Entry, Take-Profit & Stop-Loss Target Prices
* **ENTRY (`$6.33`)**: The recommended price to open the order.
* **TAKE PROFIT (`$5.96`)**: The automatic profit goal. If price hits this, the trade closes with a gain.
* **STOP LOSS (`$6.51`)**: The emergency exit price. If price moves the wrong way, the trade closes immediately to cap losses.

#### 6. Position Sizing (User Bet Math)
* **Margin Bet (`$10`)**: The cash deposit of your own money you chose to risk.
* **Pos Size (`$100`)**: The total leveraged buying power ($\$10 \times 10\text{x} = \$100$).
* **Max Risk (SL) (`-$2.93`)**: The exact dollar amount you will lose if Stop-Loss is hit.
* **Target (TP) (`+$5.86`)**: The exact dollar profit you will win if Take-Profit is hit.

#### 7. Net Expectancy, Fees & Safe Leverage
* **Gross R:R (`2:1`)**: The raw mathematical reward-to-risk ratio.
* **Net R:R (`1.89:1`)**: The true reward-to-risk ratio **after deducting Binance VIP-0 trading fees, BNB discounts, and slippage**.
* **Net EV (`+$1.34`)**: Net Expected Value in USD per trade. A positive number means this strategy makes money over thousands of iterations.
* **Sug. Leverage (`29x ✨`)**: The maximum safe leverage calculated by the AI that keeps your liquidation price at least 5% away and safely beyond your Stop-Loss. **Clicking this badge automatically applies it to your controls!**

#### 8. Specialist AI Agent Summaries
* **Tech (`SELL_SHORT 66%`)**: What the Technical Analysis Agent and 18 indicators concluded.
* **News (`BEARISH -0.50`)**: News sentiment score from $-1.0$ (panic) to $+1.0$ (euphoria).
* **Fund (`28/100`)**: Fundamental health score from CoinGecko ($0$ to $100$).

#### 9. Risk Gate Rejections Box (Only on Blocked Cards)
* If a trade is blocked, this red box explains **why**:
  * `• RULE 7 - HOLD Safety: HOLD signals are never executable` (prevents opening trades in choppy sideways markets).
  * `• RULE 1 - Direction Alignment: SELL_SHORT not allowed on SPOT market` (prevents illegal shorting on spot).

#### 10. Action Buttons (Footer)
* **`🔍 Deep Dive`**: Opens a complete modal showing all 18 indicators, candlestick patterns, support/resistance levels, order blocks, Elliott waves, and the LLM Risk Reviewer report.
* **`⚡ Re-analyze`**: Instantly re-scans this single coin with fresh real-time exchange data.
* **`🎯 Place Trade`**: Launches the trade placement modal to confirm order sizing and save it to the database (glowing green when executable; gray when blocked).

---

## 4. The "Team of Experts": Multi-Agent AI Architecture
...
## 5. The 7-Step Journey of a Trade Signal
...
sequenceDiagram
    ...
    User->>Pipe: Request Analysis (BTCUSDT, R:R=2.0, Bet=$10, Lev=10x, force_refresh=true)
    ...
    Pipe->>UI: Stream live signal, MTFA cards & exact timestamps via WebSocket
...
## 6. Multi-Timeframe Analysis (MTFA) Demystified
...
1. **`FULL_CONFLUENCE` ($\ge 85\%$ alignment with HTF and LTF confirmation)**:
   ...
   * **Reward**: Automatically boosts the trade's confidence score by **+15%** (`+0.15`).
...
## 7. The 18 Technical Indicators Explained
...
## 8. 10 Macro Patterns & 9 Candlestick Patterns
...
## 9. Smart Money Concepts (SMC / ICT) & Elliott Wave Theory
...
## 10. The 20 Deterministic Strategies & Consensus Math
...
## 11. The Unbreakable Digital Shield: The 8 Risk Rules
...
│         & Derivatives Guard    │ 6b. Liquidation price must be >= 5.0% away from entry │
│                                │     (supporting safe leverage up to 20x) and past SL. │
...
## 12. Money & Risk Management Demystified
...
4. **Pre-Trade 8-Point Audit**:
   * Before placing an order, the system checks:
     ...
     8. Is the Net R:R at least 1.5:1 after all Binance fees and slippage?

---

## 13. System Blueprint & Directory Map
...
## 14. The Web Dashboard, Live Scanning & User Experience
...
2. **🎛️ Dual-Tab Navigation Bar**:
   - **Market Signals Tab**: Real-time scanner, individual coin analyzer, multi-agent signal cards, search bar, and trade placement.
...
3. **⚡ Individual Coin Quick Analyzer**: Pick any coin among Top 50, select `BOTH (Spot + Futures)`, `SPOT`, or `FUTURES`, and click **"⚡ Analyze Coin"** for instant live feedback.
4. **📊 Real-Time Scan Progress & Completion Banner**: Watch live percentage completion and completed coin counters during 50-coin scans, with a one-click **"🔄 Re-Analyze All 50 Coins"** button upon completion.
5. **⏱️ Exact Timestamp & Relative Age Indicators**: Signal cards, inspection modals, and trade placement dialogs show exact analysis timestamps (`YYYY-MM-DD HH:MM:SS`) and relative times (`just now`, `2m ago`) that refresh automatically every 15 seconds.
6. **⚡ Per-Card Instant Re-Analyze**: Dedicated `⚡ Re-analyze` button on each card to refresh a single coin's analysis on demand.
7. **📊 6-Column Portfolio Risk Telemetry**: Live dials displaying Account Balance, Available Margin, Margin Utilization %, Open Risk Exposure ($ and %), Net Realized PnL, Win Rate %, Net Profit Factor, Kelly Sizing %, and Fee Drag %.
8. **🎯 Interactive "Place Trade" Modal**: Review multi-timeframe confirmation cards (`15m`, `1h`, `4h`), analysis timestamp, confluence badges, and execute orders with pre-trade risk verification.
9. **🔬 Deep Dive Inspection Modal**: Inspect technical indicator values, macro chart patterns, fundamental tokenomics, news headlines, and risk reviewer feedback for any coin.

---

## 15. Summary Cheat Sheet & FAQ
...
* **How does Multi-Timeframe Analysis work?** It evaluates `15m`, `1h`, and `4h` timeframes simultaneously, requiring alignment between the macro trend (4h) and trigger (15m) before applying confidence boosts (up to **+15%** for full confluence).
...
* **How many strategies and tests exist?** 20 distinct quantitative strategies and **60 automated unit & regression tests** (100% passing).
