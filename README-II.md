...
5. [The 18 Technical Indicators Explained](#5-the-18-technical-indicators-explained)
6. [10 Macro Patterns & 9 Candlestick Patterns](#6-10-macro-patterns--9-candlestick-patterns)
7. [Smart Money Concepts (SMC / ICT) & Elliott Wave Theory](#7-smart-money-concepts-smc--ict--elliott-wave-theory)
8. [The 20 Deterministic Strategies & Consensus Math](#8-the-20-deterministic-strategies--consensus-math)
9. [The Unbreakable Digital Shield: The 7 Risk Rules](#9-the-unbreakable-digital-shield-the-7-risk-rules)
10. [System Blueprint & Directory Map](#10-system-blueprint--directory-map)
11. [The Web Dashboard & User Experience](#11-the-web-dashboard--user-experience)
12. [Summary Cheat Sheet & FAQ](#12-summary-cheat-sheet--faq)

---

## 1. What is This System? (The Big Picture)

The **Multi-Agent AI Crypto Trading & Market Analysis Engine** is an automated, real-time computer software system written in **Python 3.11+**. Its purpose is to monitor cryptocurrency markets (such as Bitcoin, Ethereum, and Solana) 24/7, analyze real-time market data, coordinate specialist AI agents, test strategy consensus, and calculate precise, risk-guarded trade setups.

### 🌟 Real-World Analogy: The Formula 1 Racing Team
Think of this system like a world championship **Formula 1 Racing Team**:
* **The Telemetry Sensors (CCXT, Binance & CoinGecko APIs)** measure the car's tire pressure, engine temperature, track humidity, and lap speed in real time.
* **The Specialist Engineers (Specialist AI Agents)** each focus on one discipline:
  * *Telemetry Engineer*: Technical chart indicators and candlestick formations.
...
* **The Driver's Cockpit Dashboard (FastAPI & WebSockets)** streams live glowing telemetry dials and clear actionable signals to the team boss (you!).

```mermaid
flowchart TD
    A["🌍 Real-Time Market Ingestion<br/>(Binance Spot/Futures via CCXT, CoinGecko, crypto.news)"] --> B["🤖 Specialist AI Team<br/>(Tech, News, Fundamentals, Derivatives)"]
    B --> C["🎯 20 Strategies Consensus Engine<br/>(Requires 60% Weight & >= 5 Agreeing)"]
    C --> D["🧠 Master Coordinator AI<br/>(Entry, TP, SL, R:R, Position Sizing Math)"]
    D --> E["🕵️ Independent Risk Reviewer<br/>(Devil's Advocate AI Validation)"]
    E --> F["🛡️ Hard Risk Gate Guard<br/>(7 Deterministic Code-Level Safety Rules)"]
    F -->|Passed (All 7 Rules OK)| G["🚀 Executable Signal on Web Dashboard & CLI"]
    F -->|Blocked (Violations Found)| H["🛑 Blocked Signal (Logs Reasons for Safety)"]
```
...
graph LR
    ...
    subgraph SpecialistAgents ["2. Specialist AI Agents"]
        A1["💻 Tech Agent<br/>(18 Indicators & 19 Patterns)"]
...
* **What it does**: Computes 18 mathematical indicators, scans 10 macro chart patterns, detects 9 Japanese candlestick pattern categories, analyzes SMC/ICT order flow, and verifies Elliott Wave impulse legs.
* **Output**: A comprehensive technical breakdown, indicator readings, detected patterns, and an overall bias: `BULLISH`, `BEARISH`, or `NEUTRAL`.
...
sequenceDiagram
    ...
    Pipe->>Data: Fetch live OHLCV candlestick data & derivatives metrics
    Data-->>Pipe: Return 200 candles, funding rates, OI, and tokenomics

    par Parallel Agent Execution
        Pipe->>Agents: Run Tech Agent (18 Indicators, Patterns, SMC, Waves)
    ...
    Pipe->>UI: Stream live signal & multi-agent details via WebSocket
    UI-->>User: Display glowing interactive card + modal inspection
```

---

## 5. The 18 Technical Indicators Explained
...
## 6. 10 Macro Patterns & 9 Candlestick Patterns
...
## 7. Smart Money Concepts (SMC / ICT) & Elliott Wave Theory
...
## 8. The 20 Deterministic Strategies & Consensus Math
...
## 9. The Unbreakable Digital Shield: The 7 Risk Rules
...
## 10. System Blueprint & Directory Map

```text
my-ai-crypto-bot/
├── main.py               # 🚀 Application Launcher & CLI Single-Shot Runner
├── server.py             # 🌐 FastAPI Web Server, Auth APIs, WebSocket Streamer & UI
├── pipeline.py           # 🔄 Master Pipeline Orchestrator (coordinates everything)
├── risk_gate.py          # 🛡️ Hard-Coded 7-Rule Safety Gate Guard
├── config.py             # ⚙️ Settings, Risk Thresholds, API URLs, Monitored Coins
├── models.py             # 📋 Pydantic v2 Data Models & Schemas
├── database.py           # 💾 Supabase Cloud Database Manager (Articles & Signals)
├── ccxt_client.py        # 📡 CCXT Binance Market Data Client (Spot & Futures)
├── indicators.py         # 📐 18 Indicators, 10 Macro Patterns, 9 Candlestick Detectors
...
│   ├── tech_agent.py     # 📊 Technical Chart Analysis Agent
...
└── tests/                # 🧪 Automated Test Suite
    ├── __init__.py
    └── test_regressions.py # 27 Regression & Unit Tests
```

---

## 11. The Web Dashboard & User Experience
...
2. **⚡ Real-Time WebSocket Streaming (`/ws`)**: Price movements and AI trade signals appear automatically without page refreshes.
...
4. **🔍 Live Instant Search**: Search any coin among Top 30 monitored pairs with real-time UI filtering.
5. **🔬 Deep Dive Inspection Modal**: Click on any signal card to open a multi-tab inspection modal showing:
   * 18 technical indicators, chart patterns, and candlestick pattern breakdowns.
   * CoinGecko fundamental scores (0-100), FDV ratios, and developer commits.
   * Scraped news headlines and sentiment scores.
   * Derivatives 8h funding rates, open interest trends, and liquidation distance buffers.
   * The Risk Reviewer's feedback, concerns, and Risk Gate validation logs.
6. **📰 News Scraper Control**: Trigger on-demand news crawls directly from the web interface.
7. **🔒 Logout Control**: One-click logout locks the control center.

---

## 12. Summary Cheat Sheet & FAQ
...
* **How many strategies are there?** 20 distinct quantitative strategies aggregated through a weighted consensus engine.
